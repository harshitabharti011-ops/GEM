...
#undef GEMV1_SHUF_CLKEN_K_4
    ...
    // Apply the data_inv correction IN PLACE, before the macro phase.
    //
    // A write-out slot stores the value of pin_iv; for an inverted pin that is
    // the COMPLEMENT of the AND-gate cover's raw result, and data_inv (c3) is
    // what restores it. A macro reading shared_writeouts before this
    // correction gets the wrong polarity on every inverted operand -- the
    // whole-vector inversion originally seen on a CARRY4's O outputs.
    //
    // Macro results are written AFTER this point and carry data_inv = 0, so
    // they must not be corrected again. Doing it here rather than per-read
    // also costs nothing: it replaces the two XORs this function used to do
    // with one, so the register footprint does not grow.
    shared_writeouts[threadIdx.x] ^= t4_5.c3;
...
#undef GEM_RD_BIT
    ...
    __syncthreads();
    // Already data_inv-corrected above; macro results need no correction.
