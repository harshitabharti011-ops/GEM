    ...
    pub fn build_one(
            ...
            match edg {
                ...
                EndpointGroup::Macro(mb) => {
                    ...
                    for &in_iv in mb.in_iv.iter() {
                        ...
                        // Another macro's combinational output is not a
                        // boolean write-out; it owns a bit in the macro block
                        // already, and charging it here inflates the budget.
        ...
        let mut pending_macros = IndexSet::<usize>::new();
        {
            let probe = aig.topo_traverse_generic(
                ...
                Some(&realized_inputs));
            for &n in &probe {
                // A macro output that is ALREADY a realised input was
                // evaluated by an earlier major stage and reaches this one
                // through global state. topo_traverse_generic still emits the
                // node itself -- it only declines to recurse THROUGH it -- so
                // without this check the macro is scheduled a second time.
                //
                // That is not merely wasted work. A re-run SRLC32E shifts its
                // word state twice in one cycle, and the second evaluation
                // needs operands (its clock enable above all) that a later
                // major stage has no reason to produce, which is where the
                // "clock-enable aigpin has no boomerang write-out position"
                // panic comes from.
                if realized_inputs.contains(&n) { continue }
        ...
        //
        // TWO passes, and the order is load-bearing. A chained CARRY4 takes
        // CO[3] of the previous one as its CIN, so that pin is both a macro
        // OUTPUT and another macro's INPUT. Doing insert-then-remove per cell
        // in a single pass leaves it in the work list whenever the producer is
        // visited before the consumer -- which is always, since the probe
        // walks in topological order. The boolean hierarchy then tries to
        // cover a pin no AND gate can produce, and build_one_boomerang_stage
        // panics with "unscheduled combinational macro node".
        for &cellid in pending_macros.iter() {
            for i in aig.macros[&cellid].comb_inputs() {
                unrealized_comb_outputs.insert(i);
            }
        }
        for &cellid in pending_macros.iter() {
            for &o in aig.macros[&cellid].outputs.iter() {
        ...
        while !unrealized_comb_outputs.is_empty() || !pending_macros.is_empty() {
            ...
            if !ready.is_empty() {
                ...
                //
                // The pending_macros check is essential, not defensive. On the
                // LAST iteration of any partition the boolean work list drains
                // to empty, and in a macro-free partition `ready` is empty too
                // -- so without it this rejects every ordinary partition in
                // the design, the partitioner keeps doubling the part count,
                // and mapping never converges.
                clilog::error!(
